...
/**
  * This is an event sourced entity. It has a state, [[LoudamState]], which
  ...
  */
class LoudamEntity extends PersistentEntity {

  override type Command = LoudamCommand[_]
  override type Event = LoudamEvent
  override type State = LoudamState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: LoudamState = LoudamState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case LoudamState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        LoudamState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class LoudamState(message: String, timestamp: String)
...
object LoudamState {
  /**
    ...
    */
  implicit val format: Format[LoudamState] = Json.format
}

/**
  * This interface defines all the events that the LoudamEntity supports.
  */
sealed trait LoudamEvent extends AggregateEvent[LoudamEvent] {
  def aggregateTag = LoudamEvent.Tag
}

object LoudamEvent {
  val Tag = AggregateEventTag[LoudamEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LoudamEvent
...
/**
  ...
  */
sealed trait LoudamCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends LoudamCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends LoudamCommand[String]
...
/**
  ...
  * application loader.
  */
object LoudamSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LoudamState]
  )
}
